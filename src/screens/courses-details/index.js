...
class CoursesDetails extends Component {
  ...
  fetchCertificateUrl = async (courseId, userId) => {
    try {
      ...
      if (response.ok && data?.certificate_url) {
        ...
        console.log(data?.message, 'error1');
        console.log(data?.message, data?.message1, 'error');
        // Handle API error or missing certificate URL
        Alert.alert(
          'Error',
          data?.message ||
      ...
      Alert.alert(
        'Error',
        'Unable to fetch the certificate URL. Please try again later.',
      );
    }
  };
  ...
  handleShowCertificate = () => {
    ...
    if (courseId && userId) {
      this.fetchCertificateUrl(courseId, userId);
    } else {
